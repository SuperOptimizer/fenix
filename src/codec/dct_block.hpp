...
inline const std::array<ScanEntry, kDctN * kDctN * kDctN>& dct_scan_order() {
    static const std::array<ScanEntry, kDctN * kDctN * kDctN> s = [] {
        ...
        for (u32 i = 0; i < a.size(); ++i) {
            const int z = static_cast<int>(i) / (kDctN * kDctN), y = (static_cast<int>(i) / kDctN) % kDctN,
                      x = static_cast<int>(i) % kDctN;
            a[i] = {
                static_cast<u16>(i),
                static_cast<u8>(z + y + x),
...
inline void dct_step_table(const DctParams& p, f32* stepv) {
    for (int f = 0; f < 3 * kDctN; ++f)
        stepv[f] = p.q * std::pow(1.0f + static_cast<f32>(f), p.hf_exp);
}
...
template<class V> inline s32 causal_block_pred(const V* a, s64 bz, s64 by, s64 bx, s64 bpa) {
...
inline std::pair<s32, u32> quant_one_block(
    ...
    for (f32& v : c)
        v -= static_cast<f32>(dc_level) * dc_step;
    dct16_3d_fwd(c);
    for (s64 z = 0; z < N; ++z)
        for (s64 y = 0; y < N; ++y)
            for (s64 x = 0; x < N; ++x) {
    ...
    for (usize pos = static_cast<usize>(V); pos-- > 0;)
        if (lev[scan[pos].idx] != 0) {
            nsig = static_cast<u32>(pos + 1);
            break;
        }
...
inline int
cluster_contexts(const std::array<std::array<u32, kDctCat>, kDctRawCtx>& hist, std::array<u8, kDctRawCtx>& cluster_of) {
    auto cost = [](const std::array<u32, kDctCat>& h) -> f64 {
        ...
        for (u32 c : h)
            if (c)
                ent += static_cast<f64>(c) * std::log2(n / static_cast<f64>(c));
    ...
    while (nactive > 1) {
        ...
        for (int i = 0; i < kDctRawCtx; ++i) {
            ...
            for (int j = i + 1; j < kDctRawCtx; ++j) {
                ...
                for (int s = 0; s < kDctCat; ++s)
                    m[static_cast<usize>(s)] = ch[i][static_cast<usize>(s)] + ch[j][static_cast<usize>(s)];
        ...
        if (bi < 0 || (!force && best >= 0))
            break; // no beneficial merge AND within the cluster cap
        for (int s = 0; s < kDctCat; ++s)
            ch[bi][static_cast<usize>(s)] += ch[bj][static_cast<usize>(s)];
        ccost[bi] = cost(ch[bi]);
        alive[bj] = false;
        for (int r = 0; r < kDctRawCtx; ++r)
            if (cluster_of[r] == bj)
                cluster_of[r] = static_cast<u8>(bi);
        --nactive;
    }
    ...
    int K = 0;
    for (int r = 0; r < kDctRawCtx; ++r)
        if (alive[cluster_of[r]] && remap[cluster_of[r]] < 0)
            remap[cluster_of[r]] = K++;
    if (K == 0)
        K = 1;
    for (int r = 0; r < kDctRawCtx; ++r)
...
inline void deblock_tile(std::span<f32> t, s64 side, const DctParams& p) {
    ...
    auto smooth = [&](f32 p2, f32 p1, f32& p0, f32& q0, f32 q1, f32 q2) {
        ...
        if (dp + dq >= beta)
            return; // not flat on both sides → a real feature crosses here, leave it
        f32 delta = (9.0f * (q0 - p0) - 3.0f * (q1 - p1)) * (1.0f / 16.0f); // HEVC weak-filter delta
        if (std::abs(delta) >= 10.0f * tc)
            return; // jump far larger than the step → a real edge, not ring
    ...
    for (s64 b = kDctN; b < side; b += kDctN) { // interior boundaries only
        for (s64 i = 0; i < side; ++i)
            for (s64 j = 0; j < side; ++j) {
                smooth(
                    at(b - 3, i, j), at(b - 2, i, j), at(b - 1, i, j), at(b, i, j), at(b + 1, i, j), at(b + 2, i, j)
                ); // z-edges
                smooth(
                    at(i, b - 3, j), at(i, b - 2, j), at(i, b - 1, j), at(i, b, j), at(i, b + 1, j), at(i, b + 2, j)
                ); // y-edges
                smooth(
                    at(i, j, b - 3), at(i, j, b - 2), at(i, j, b - 1), at(i, j, b), at(i, j, b + 1), at(i, j, b + 2)
                ); // x-edges
            }
    }
}
...
template<class T> inline std::vector<u8> encode_tile_dct(std::span<const T> tile, s64 bpa, DctParams params = {}) {
    ...
    for (s64 b = 0; b < nblk; ++b) {
        ...
        const auto [dl, ns] =
            detail::quant_one_block<T>(tile, side, bz * N, by * N, bx * N, params, stepv, lev, coefall.data() + b * V);
    ...
    if (params.rdoq) {
        ...
        for (int r = 0; r < kDctRawCtx; ++r)
            for (int cat = 0; cat < kDctCat; ++cat)
                chist[cmap[static_cast<usize>(r)]][static_cast<usize>(cat)] +=
                    hist[static_cast<usize>(r)][static_cast<usize>(cat)];
        for (int cl = 0; cl < K; ++cl) {
            f64 tot = 0;
            for (f64 v : chist[static_cast<usize>(cl)])
                tot += v;
            for (int cat = 0; cat < kDctCat; ++cat)
                bc[static_cast<usize>(cl)][static_cast<usize>(cat)] = static_cast<f32>(
                    -std::log2((chist[static_cast<usize>(cl)][static_cast<usize>(cat)] + 0.5) / (tot + 0.5 * kDctCat))
                );
        }
        for (s64 b = 0; b < nblk; ++b) {
            s32* lev = levall.data() + b * V;
            ...
            for (u32 pos = 0; pos < nsigs[static_cast<usize>(b)]; ++pos) {
                ...
                auto rd = [&](u32 m) {
                    ...
                    return (ac - dq) * (ac - dq) +
                           lam * (bc[cl][static_cast<usize>(cat)] + static_cast<f32>(m ? cat : 0));
                };
                u32 bm = m0;
                f32 best = rd(m0);
                if (const f32 c0 = rd(0); c0 < best) {
                    best = c0;
                    bm = 0;
                }
                if (m0 >= 2)
                    if (const f32 cl1 = rd(m0 - 1); cl1 < best) {
                        best = cl1;
                        bm = m0 - 1;
                    }
    ...
    for (int r = 0; r < kDctRawCtx; ++r)
        out.push_back(cmap[static_cast<usize>(r)]); // context map
    ...
    for (int cl = 0; cl < K; ++cl)
        detail::encode_plane(out, cats[static_cast<usize>(cl)]);
    bits.flush();
...
// Decode an encode_tile_dct payload back to the contiguous (bpa·kDctN)³ region of dtype T. UNTRUSTED
// INPUT: `payload` may be a corrupt/adversarial .fxvol tile blob that has only passed the archive's CRC
// check (which detects torn writes, not a crafted file) — every decoded field (nsigs, K, the context map,
// the category symbols, the raw magnitude-category byte) is validated before it drives an index, a shift,
// or a slot write, per the module's hard rule "no UB/crash on any bytes" (codec/CLAUDE.md). Returns an
// error instead of trusting any of it; wrong-but-safe output is fine, a crash is not.
template<class T>
inline Expected<std::vector<T>> decode_tile_dct(std::span<const u8> payload, s64 bpa, DctParams params = {}) {
    constexpr s64 N = kDctN, sz = N * N, V = N * N * N;
    const s64 side = bpa * N, nblk = bpa * bpa * bpa;
    usize p = 0;
    std::vector<s32> dclev(static_cast<usize>(nblk));
    std::vector<u32> nsigs(static_cast<usize>(nblk));
    for (s64 b = 0; b < nblk; ++b) {
        auto z_r = detail::get_var(payload, p);
        if (!z_r)
            return std::unexpected(z_r.error());
        const u32 z = *z_r;
    ...
    for (s64 b = 0; b < nblk; ++b) {
        ...
        const s64 bz = b / (bpa * bpa), by = (b / bpa) % bpa, bx = b % bpa;
        const s32 pred = params.dc_predict ? detail::causal_block_pred(nsigs.data(), bz, by, bx, bpa) : 0;
        const s64 ns = static_cast<s64>(res) + pred;
        if (ns < 0 || ns > V)
            return err(Errc::decode_error, "nsig out of range"); // clamp to V=4096 (see below)
        nsigs[static_cast<usize>(b)] = static_cast<u32>(ns);
    }
    if (p >= payload.size())
        return err(Errc::decode_error, "truncated tile header (K)");
    const int K = payload[p++];
    if (K <= 0 || K > kDctMaxClusters)
        return err(Errc::decode_error, "K out of range");
    if (p + kDctRawCtx > payload.size())
        return err(Errc::decode_error, "truncated context map");
    std::array<u8, kDctRawCtx> cmap;
    for (int r = 0; r < kDctRawCtx; ++r) {
        const u8 v = payload[p++];
        if (v >= K)
            return err(Errc::decode_error, "context map entry out of range");
        cmap[static_cast<usize>(r)] = v;
    }
    std::vector<std::vector<u8>> cats(static_cast<usize>(K));
    std::vector<usize> cur(static_cast<usize>(K), 0);
    for (int cl = 0; cl < K; ++cl) {
        auto plane = detail::decode_plane(payload, p, static_cast<u32>(nblk * V));
        if (!plane)
            return std::unexpected(plane.error());
        for (u8 cat : *plane)
            if (cat >= kDctCat)
                return err(Errc::decode_error, "category symbol out of range");
        cats[static_cast<usize>(cl)] = std::move(*plane);
    }
    auto nb_r = detail::get_var(payload, p);
    if (!nb_r)
        return std::unexpected(nb_r.error());
    const u32 nb = *nb_r;
    if (p + nb > payload.size())
        return err(Errc::decode_error, "truncated bit stream");
    ...
    for (s64 b = 0; b < nblk; ++b) {
        ...
        // nsigs[b] was already range-checked (<= V) above, so `scan[pos]` below stays in-bounds.
        for (u32 pos = 0; pos < nsigs[static_cast<usize>(b)]; ++pos) {
            const detail::ScanEntry e = scan[pos];
            const usize i = e.idx;
            const int nsum = ((e.edge & 1) ? sig[i - static_cast<usize>(sz)] : 0) +
                             ((e.edge & 2) ? sig[i - static_cast<usize>(N)] : 0) + ((e.edge & 4) ? sig[i - 1] : 0);
            const int cl = cmap[static_cast<usize>(detail::dct_raw_ctx(e.fsum, nsum))];
            if (cur[static_cast<usize>(cl)] >= cats[static_cast<usize>(cl)].size())
                return err(Errc::decode_error, "category stream underrun");
            const int cat = cats[static_cast<usize>(cl)][cur[static_cast<usize>(cl)]++]; // already < kDctCat
            if (cat > 0) {
    ...
    if (params.deblock)
        detail::deblock_tile(recon, side, params);
    for (usize i = 0; i < recon.size(); ++i)
        out[i] = from_f32_one<T>(recon[i]);
    return out;
}

// Single-block codec = a tile of one block (byte-compatible with the per-block format).
template<class T> inline std::vector<u8> encode_block_dct(std::span<const T> block, DctParams params = {}) {
    return encode_tile_dct<T>(block, 1, params);
}

template<class T>
inline Expected<std::vector<T>> decode_block_dct_to(std::span<const u8> payload, DctParams params = {}) {
...
FENIX_CODEC_EXTERN template Expected<std::vector<u8>> decode_tile_dct<u8>(std::span<const u8>, s64, DctParams);
FENIX_CODEC_EXTERN template Expected<std::vector<f32>> decode_tile_dct<f32>(std::span<const u8>, s64, DctParams);
