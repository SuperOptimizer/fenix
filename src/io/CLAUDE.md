...
- **OME-Zarr reader** (`zarr.hpp`) — v2 (`.zarray`) **and** v3 (`zarr.json`), including v3
  **sharding_indexed** (index_location=end, optional crc32c trailer); raw + gzip + blosc2
  + **dct3d** chunk codecs (blosc needs `FENIX_HAVE_BLOSC2`; raw-zstd unsupported — typed
  reject). `dct3d.hpp` is a fresh decode-only C++ port of the SuperOptimizer/3ddct 16^3
  lossy block codec (matter-compressor lineage; the dl.ash2txt.org community exports are
  zarr v3 shards of 16^3 dct3d inner chunks at q=32/tau=64 — ~15-25x fewer WAN bytes than
  the raw open-data bucket at the quality grade training already uses). u8/u16/f32; the
  16^3 geometry is enforced at meta parse (typed reject otherwise); golden test blobs in
  tests/test_io.cpp are generated by the reference C encoder (tolerance 1 vs its decoder).
