...
inline Expected<int> ingest_zarr(std::span<const std::string_view> args, Context&) {
    ...
    log(LogLevel::info,
        "ingest-zarr: {} level {} region z{}:{} y{}:{} x{}:{} -> {}",
        root,
        level,
        origin.z,
        origin.z + extent.z,
    ...
    if (outpath.size() > 5 && outpath.substr(outpath.size() - 5) == ".zarr") {
        if (auto r = copy_zarr_region_local(lroot, outpath, origin, extent); !r) return std::unexpected(r.error());
        log(LogLevel::info,
            "ingest-zarr: wrote local zarr {} ({}x{}x{} ZYX, raw chunk copy)",
            outpath,
            extent.z,
            extent.y,
            extent.x);
...
inline Expected<int> export_scroll(std::span<const std::string_view> args, Context&) {
    if (args.size() < 3) {
        log(LogLevel::error,
            "usage: fenix export-scroll <zarr-root|url> <level> <out.fxvol> [q=8] [region=256] [z0 y0 x0 D H W]");
        return err(Errc::invalid_argument, "missing args");
    }
    ...
    auto parse_f = [](std::string_view s, f32 d) {
        f32 v = d;
        std::from_chars(s.data(), s.data() + s.size(), v);
        return v;
    };
    ...
    for (s64 k = 5; k >= 1; --k) {
        ...
        log(LogLevel::info,
            "air-skip: occupancy from level {} ({}x{}x{}, scale {})",
            level_int + k,
            occ.dims().z,
            occ.dims().y,
            occ.dims().x,
    ...
    Expected<codec::VolumeArchive> ar =
        resume ? codec::VolumeArchive::open(out, true) : codec::VolumeArchive::create(out, shape, {.q = q});
    ...
    const s64 rz1 = ndiv(std::min(b0.z + bd.z, shape.z), R), ry1 = ndiv(std::min(b0.y + bd.y, shape.y), R),
              rx1 = ndiv(std::min(b0.x + bd.x, shape.x), R);
    const s64 total = (rz1 - rz0) * (ry1 - ry0) * (rx1 - rx0);
    log(LogLevel::info,
        "export-scroll {} L{} ({}x{}x{}) -> {} q={} region={} commit={}  {} regions  ({})",
        root,
        std::string(args[1]),
        shape.z,
        shape.y,
    ...
    for (s64 rz = rz0; rz < rz1; ++rz)
        for (s64 ry = ry0; ry < ry1; ++ry)
            for (s64 rx = rx0; rx < rx1; ++rx) {
                ...
                if (a.coverage(0, ft) != codec::Coverage::Absent) {
                    ++skipped;
                    continue;
                }  // 1st tile present = done
                if (!occupied(org, ext)) {
                    ++skipped_air;
                    continue;
                }  // all-air per the coarse map → leave ABSENT
                work.push_back({org, ext});
            }
    const s64 nwork = static_cast<s64>(work.size());
    log(LogLevel::info,
        "  worklist: {} regions to fetch ({} air-skip, {} resume-skip, of {} in box)",
        nwork,
        skipped_air,
        skipped,
        total);
    ...
    int nprod = 8;
    if (const char* e = std::getenv("FENIX_EXPORT_PREFETCH")) {
    ...
    auto producer = [&] {
        for (;;) {
            ...
            for (int attempt = 1; !reg && attempt <= 20; ++attempt) {
                ...
                log(LogLevel::warn,
                    "region z{} y{} x{} read failed: {} — retry {}/20 in {}s",
                    w.org.z,
                    w.org.y,
                    w.org.x,
                    reg.error().message,
    ...
    for (;;) {
        ...
        if (const auto nowt = clk::now(); std::chrono::duration<f64>(nowt - t_last).count() >= 15.0 || done == 0) {
            ...
            log(LogLevel::info,
                "  {}/{} fetched ({:.1f}%) {:.0f}s ETA {:.0f}s | buffered {}/{} | air-skip {} resume-skip {} | real {} "
                "zero {} tiles | {:.1f}MiB",
                done,
                nwork,
                100.0 * frac,
            ...
            for (s64 ty = 0; ok && ty < ndiv(ext.y, T); ++ty)
                for (s64 tx = 0; ok && tx < ndiv(ext.x, T); ++tx) {
                    for (s64 z = 0; z < T; ++z)  // extract a 64³ tile, edge-replicating at the box edge
                        for (s64 y = 0; y < T; ++y)
                            for (s64 x = 0; x < T; ++x)
                                block[static_cast<usize>((z * T + y) * T + x)] = rv(std::min(tz * T + z, ext.z - 1),
                                                                                    std::min(ty * T + y, ext.y - 1),
                                                                                    std::min(tx * T + x, ext.x - 1));
                    const ChunkCoord tc{org.z / T + tz, org.y / T + ty, org.x / T + tx};
                    if (auto w = a.write_chunk(0, tc, block); !w) {
                        consume_err = std::unexpected(w.error());
                        ok = false;
                        break;
                    }
        ...
        if (ok && ++since_commit >= commit_every) {  // batched crash-safe checkpoint (see commit_every above)
            if (auto c = a.commit(); !c) {
                consume_err = std::unexpected(c.error());
                ok = false;
            }
            since_commit = 0;
        }
    ...
    if (auto c = a.close(); !c) return std::unexpected(c.error());
    log(LogLevel::info,
        "export-scroll done: {} regions ({} air-skipped, {} resume-skipped), real {} / zero {} tiles, {:.1f} MiB",
        total,
        skipped_air,
        skipped,
        real_tiles,
...
inline Expected<int> info_vol(std::span<const std::string_view> args, Context&) {
    if (args.size() < 1) {
        log(LogLevel::error, "usage: fenix fxinfo <in.fxvol|.fxsurf> [--json]");
        return err(Errc::invalid_argument, "missing args");
    }
    if (std::string_view(args[0]).ends_with(".fxsurf")) {
        const bool json = args.size() > 1 && args[1] == "--json";
        auto s = read_fxsurf(std::string(args[0]));
        ...
        if (json)
            std::printf("{\"type\":\"fxsurf\",\"nu\":%lld,\"nv\":%lld,\"valid\":%lld,\"scale_u\":%.3f,"
                        ...
                        static_cast<double>(hi.z),
                        static_cast<double>(hi.y),
    ...
    std::printf("fxvol %s\n  dims (ZYX) %lldx%lldx%lld   q=%.2f   LODs=%u   %s\n  data %.2f MiB (file %.2f MiB)   "
                "ratio8 %.1fx\n",
                std::string(args[0]).c_str(),
                (long long)d.z,
                (long long)d.y,
                (long long)d.x,
                static_cast<double>(a->params().q),
                a->nlods(),
                a->data_offset() ? "SEALED" : "live",
                static_cast<double>(csz) / (1024.0 * 1024.0),
                static_cast<double>(fsz) / (1024.0 * 1024.0),
    ...
    for (s64 lod = 0; lod < static_cast<s64>(a->nlods()); ++lod) {
        ...
        for (s64 cz = 0; cz < ce.z; ++cz)
            for (s64 cy = 0; cy < ce.y; ++cy)
                for (s64 cx = 0; cx < ce.x; ++cx) switch (a->coverage(lod, {cz, cy, cx})) {
...
inline Expected<int> fxupgrade(std::span<const std::string_view> args, Context&) {
    ...
    if (args.size() >= 2) {
        ...
        if (s == "u8")
            ...
            return err(Errc::invalid_argument, "bad dtype (u8|u16|u32|s8|s16|s32|f16|f32)");
    }
    ...
    for (int slot = 0; slot < 2; ++slot) {
        u8 buf[kFxSuper];
        if (std::fseek(f, static_cast<long>(slot * kFxSuper), SEEK_SET) != 0) {
            std::fclose(f);
            return err(Errc::io_error, "seek");
        }
        if (std::fread(buf, 1, kFxSuper, f) != kFxSuper) {
            std::fclose(f);
            return err(Errc::io_error, "short read");
        }
        u32 magic, ver;
        ...
        if (ver == codec::fxvol_version) {
            ++already;
            continue;
        }
        if (ver != 4) {
            std::fclose(f);
            return err(Errc::unsupported, "only v4 → v5 upgrade is supported");
        }
        ...
        if (std::fwrite(buf, 1, kFxSuper, f) != kFxSuper) {
            std::fclose(f);
            return err(Errc::io_error, "write");
        }
        ++patched;
    }
    std::fflush(f);
    ...
    log(LogLevel::info,
        "fxupgrade: {} → v{} ({} superblock(s) patched, dtype={})",
        path,
        codec::fxvol_version,
        patched,
        static_cast<int>(static_cast<u8>(dt)));
    return 0;
}
...
[[maybe_unused]] const int fenix_stage_export_scroll = ::fenix::register_stage(::fenix::Stage{
    "export-scroll", "stream a whole OME-Zarr level into .fxvol (out-of-core, resumable)", ::fenix::io::export_scroll});
[[maybe_unused]] const int fenix_stage_transcode = ::fenix::register_stage(::fenix::Stage{
