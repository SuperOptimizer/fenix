...
inline Expected<int> run_surf_consist(std::span<const std::string_view> args, Context&) {
    ...
    for (usize a = 0; a < infos.size(); ++a) {
        ...
        for (usize b2 : partners) {
            ...
            // DISAGREE (was CROSS): needs med ABOVE the duplicate-trace wobble scale — measured
            // on a flagged PHercParis4 pair (med 1.3, coinc 0.66), the signed-distance histogram
            // is unimodal at 0 (69% within ±2 vox) = same sheet traced twice, not interpenetration.
            // And the corpus-wide sweep showed the med 2-4 side-mixed population is REDUNDANT
            // COVERAGE disagreeing by 2-3 vox (two full tracings of wraps 10-27 + per-wrap segs +
            // literal *_copy / v14 re-registrations), not physically-impossible crossings: true
            // different-wrap pairs sit ~10-30 vox apart and never enter near=6 at all. Semantics:
            // at least one of the two redundant traces is locally misregistered — actionable as
            // DEDUP (keep the better-graded version) / low-confidence, NOT quarantine-both.
            if (side_mix > 0.15 && med >= 2.0f && med < 4.0f && coinc < 0.75 && n_signed > 50) verdict = "DISAGREE";
