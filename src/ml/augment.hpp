...
inline f32 uniform(Pcg32& rng, f32 lo, f32 hi) {
    return lo + (hi - lo) * rng.next_f32();
}
        ...
        parallel_for_z(nd, [&](s64 z) {
            for (s64 y = 0; y < nd.y; ++y)
                for (s64 x = 0; x < nd.x; ++x) {
                    ...
                    i[static_cast<usize>(p[0])] = o[0];
                    i[static_cast<usize>(p[1])] = o[1];
                    i[static_cast<usize>(p[2])] = o[2];
...
// P^3 contiguous buffer (cubes are permutation-closed, so in/out dims match). Single-threaded —
// feed workers are already the parallelism.
inline void octahedral_u8_inplace(u8* buf, s64 P, int sym) {
    static const int kPerm[6][3] = {{0, 1, 2}, {0, 2, 1}, {1, 0, 2}, {1, 2, 0}, {2, 0, 1}, {2, 1, 0}};
    const int* p = kPerm[(sym / 8) % 6];
    const int fm = sym % 8;
    std::vector<u8> tmp(static_cast<usize>(P * P * P));
    for (s64 z = 0; z < P; ++z)
        for (s64 y = 0; y < P; ++y)
            for (s64 x = 0; x < P; ++x) {
                std::array<s64, 3> o{z, y, x};
                std::array<s64, 3> i{};
                i[static_cast<usize>(p[0])] = o[0];
                i[static_cast<usize>(p[1])] = o[1];
                i[static_cast<usize>(p[2])] = o[2];
                ...
                if (fm & 2) i[1] = P - 1 - i[1];
                if (fm & 4) i[2] = P - 1 - i[2];
                tmp[static_cast<usize>((z * P + y) * P + x)] = buf[static_cast<usize>((i[0] * P + i[1]) * P + i[2])];
            }
    ...
    parallel_for_z(d, [&](s64 z) {
        for (s64 y = 0; y < d.y; ++y)
            for (s64 x = 0; x < d.x; ++x) {
                ...
                const f32 ddz = sample_trilinear(czv, g), ddy = sample_trilinear(cyv, g),
...
inline void intensity(Sample& s, u64 seed, const IntensityParams& p = {}) {
    ...
    for (s64 i = 0; i < n; ++i) {
        const f32 v = s.image.flat()[static_cast<usize>(i)];
        mn = std::min(mn, v);
        mx = std::max(mx, v);
    }
...
inline void augment(Sample& s, u64 seed, const Policy& pol = {}) {
    ...
    {
        Pcg32 r(sub(1));
        octahedral(s, static_cast<int>(r.bounded(48)));
    }
